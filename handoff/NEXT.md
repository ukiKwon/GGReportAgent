...
- **남은 sub-project**:
  ...
  - **5 통합 프런트 → "기관인텔리" 멀티에이전트 협업 시스템으로 재정의**(2026-07-31,
    ...
    - ~~**A1 오케스트레이터 그래프 코어**~~ — **완료**(2026-07-31 Session 02:51,
      ...
      재시도(항목 5와 연동).
    - **A2 (다음 1순위)** — 대화창 consult(`POST /chat`)·notifications 라우터·
      업로드 즉시검사·완료 아카이브+지식 색인. **계획 미작성.** 착수 시 최종 리뷰
      이월 항목(2026-07-31_summary.md Session 02:51 §3의 F4~F10 등)을 계획에 반영할 것
      — 특히 F7(결재요청 알림)·F10(X-User-Id를 body로)은 A2 범위와 직결.
    - **B store→API 전환** — 설계 승인됨(정적 마운트+PUT+서버/폴백 모드), 계획 미작성.
    - **C 협업 UI**(화면 6종: 상황판·대화창·현황판·배점표 매핑·쪽지함·지식탭) —
      계획 미작성, 필요 시 C1/C2 재분할.
