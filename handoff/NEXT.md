...
- **남은 하위 항목**:
  1. **서울 노원·성북** — eminwon 경로 미확보. 노원은 시스템 자체가 다른 것으로
     추정(DNS 후보 3개 실패), 성북은 자체 게시판(`www.sb.go.kr/www/selectEminwonList.do`)은
     찾았으나 제목 검색 파라미터가 안 먹어 최신순 전체 목록(3만 건대)만 반환된다.
     다음에 이어받으면 검색 폼 파라미터를 다시 확인하거나 날짜 역순 페이징으로
     "금고" 포함 행만 걸러야 한다.
  2. **서울 음성 14구 재조회(9월 접수철)** — 종로·중구·용산·광진·성동·중랑·마포·
     양천·구로·영등포·동작·관악·서초·강동. 2022년 회차가 8월 공고→9월 중순
     접수였으므로, 지금(8월 초)은 "아직 공고 전"이라 안 보였을 가능성이 높다.
     9월 중순에 §⑨ 방법(eminwon navigate+fetch)을 그대로 반복하면 더 많이 잡힐 것.
