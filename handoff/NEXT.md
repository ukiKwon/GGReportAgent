...
### 1. 25개 자치구 배치 프로젝트 — 미완결 1/20(강동) 재개
- **출처**: `handoff_old/NEXT.md`(2026-07-20 기준, git 미추적 상태로 남아있던 별도
  ...
  강남 1건 추가 완료, 18/20까지 진행. 이후 `2026-07-27` 세션(`## Session 01:39`)에서
  송파 1건 추가 완료, **19/20**까지 진행.
- **프로젝트 개요**: 서울시 25개 자치구 전체에 대해 동일한 3종 산출물(`spec/`, `plan/`,
  `bank_ideas_draft.txt`)을 작성. **완료 19/20**(종로78, 중구77, 용산76, 성동76,
  중랑81, 성북76, 은평74, 강북72, 서대문73, 마포80, 양천84, 강서80, 구로84, 금천84,
  영등포81, 관악81, 서초84, 강남79, **송파69** — 신뢰도 점수).
  **미완결 1/20: 강동(gangdong) — 이것 하나로 25개 자치구 배치 프로젝트가 완결된다.**
  산출물 폴더(`giganlist/{구영문명}/spec,plan,bank_ideas_draft.txt`)는 각 세션에서
  main에 git commit·push됨(이전에는 미추적 상태였음 — 아래 "비고" 참고).
- **송파 완료 이력**: `2026-07-27` 세션(`## Session 01:39`)에서 1건 단독 디스패치, 중단 없이
  1회 완주. 산출물 검증 통과 — spec 10개(`00_인덱스`~`09_민원게시판_2026년정리`, 송파구가
  2026년 예산 보도자료에서 자체 사용한 5개 분야 채택: 복지보건일자리소상공인/안전교통환경
  도시기반시설/문화예술체육/교육/일반행정)/plan 6개/bank_ideas_draft.txt(복수형, 5축 11개
  아이디어). **신뢰도 69/100 — 배치 전체 최저치**(기존 최저 강북72보다 낮음). 단, 인용
  정확성은 24/25로 **인용 오류 0건**이고 점수를 끌어내린 건 근거 데이터 강도 9/25 —
  즉 조사 부실이 아니라 **송파구 공개 범위의 한계**이며 문장 수정으로 올릴 수 없다고
  plan/05에 명시돼 있음(재작업 지시 금지, 새 조사만이 답). 핵심 수치: 2026년 총예산
  1조 3,040억, 교차확인 확인됨10/부분확인10/확인안됨15(=35건, 42.9% 확인안됨),
  **개별 사업 예산 확인 0/35(0%)**, 담당부서 확인 2/35(5.7%), 2026년 민원 통계 전무.
  "35건은 송파구 총 사업 건수가 아니다(보도자료 대표사업+홈페이지 확인 추가분)"를 spec/00에
  경고와 함께 명시. 불일치 5건 병기(분야합 13,039≠발표 13,040 등), 에이전트가 조작 시도
  3건을 스스로 차단(서울시 융자조건을 송파 값으로 전용/상품권 설 회차 60억을 연간액 오용/
  정비사업 31≠41 오독)하고 담당부서 오표기 1건을 "(추정)"으로 하향 정정. 은행 관점 최대
  발견: **송파 지역사랑상품권 라인업에 타 금융기관 운영 배달앱 공동 브랜딩 상품권이 이미
  존재** → 관련 제안(FN-5) D등급 하향 후 "미점유 영역(가맹점 조기 정산)만 남겨 축소 재설계"로
  정정(브랜드명은 spec에만 원문추적 기록, bank_ideas 본문 미사용 — grep 검증 완료).
