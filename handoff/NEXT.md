...
- **남은 하위 항목**:
  ...
  3. **경기 26개 미확보 시 — eminwon 직접조회로 전환** — 이번엔 사용자가 잠든
     동안이라 브라우저 확장 도메인 허용을 못 받아 순수 웹 검색만 썼다(그래서
     날짜 확보율이 3/31로 서울(9/25)보다 낮다). **다음 세션은 서울과 같은 방법
     (eminwon navigate+fetch)을 경기 31개 시청에도 시도하는 것부터 시작** —
     통하는지 자체가 첫 확인 사항이고, 도메인 허용을 사용자에게 실시간으로
     받아야 하므로 사용자가 있을 때 진행해야 한다.
...
- **다음 세션 착수 순서 추천**: 사용자 지시 순서가 서울→경기→부산이었으므로
  ③(경기 eminwon 전환)을 먼저 마무리한 뒤 ④(부산)로 넘어가는 것이 자연스럽다.
