...
- **다음 세션 착수 순서 추천**: ③·④가 끝나 지시받은 지역은 전부 완료됐다.
  남은 것은 소형 항목들 — ①(노원·성북 경로)·부산 서구 승격(④에 기재)은 자투리
  시간용, ⑤(전남광주 폴리곤)·⑥(TOP5 중복)은 사용자 결정 대기.
  단, ②(서울 9월 재조회)는 **시점이 고정돼 있어**(9월 중순) 그 시점이 오면
  순서와 무관하게 우선해야 한다. 새 지역(인천·대구 등)은 사용자 지시가 있을 때
  §⑬ 절차(geo 파일 + eminwon)를 그대로 반복하면 된다.
