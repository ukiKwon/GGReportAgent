...
### 14. 전국 지자체 금고 입찰 일정 대시보드 — 서울→경기→부산 지시 순서 완료, 잔여 소형 항목만 (2026-08-10)
...
- **다음 세션 착수 순서 추천**: ③·④·①·⑤·⑥과 부산 서구 승격까지 전부 끝났고,
  **2026-08-10 오후 사용자 지시로 전국 확장 1차분(광역시)도 완료**됐다 —
  ...
  남은 것:
  7. **도 지역 확장 — 강원·충북·충남·전북·전남광주·경북 완료, 2개 지역 잔여**
     (§⑰ 남은 일 15, 강원 §⑱·§⑲ · 충북 §⑳ · 충남 §㉑ · 전북 §㉒ ·
     전남광주 §㉓ · 경북 §㉔ · `2026-08-11_summary.md`·`2026-08-12_summary.md`).
     잔여: **경남 18 · 제주 2**(행정시 — 자체 금고 여부 확인 선행). 같은 절차:
     `build_region_geo.js`(scratchpad — 없어지면 §⑬에서 재현, EXCLUDES 인자
     추가돼 있다: `'["37310"]'` 형태로 kostat 코드 배제 가능. 경남 prefix 38 ·
     제주 39) →
     DNS(`eminwon.{도메인}.go.kr`) → eminwon 전수(§⑨, 목록 tr 필터는 td
     4~12개 — 평창 유형. 일반구가 있는 시(창원)는 경기 ⓐ안. 약어
     도메인은 게재제호로 소속 확인 — jc=진천·hc=합천) → 레코드+테스트.
     접속 불가 유형 6종 총정리는 §㉓ 참조(예천 WAF는 구례 유형).
     **경북에서 배운 것(§㉔)**: ⓐ https 인증서 오류 호스트는 http 진입(의성
     eminwon.usc.go.kr) ⓑ 서버 응답 지연 시 fetch에 AbortController 35초 타임아웃
     후 재시도(구미) ⓒ 확장 세션이 끊기면 tabs_context_mcp{createIfEmpty}로 재연결.
     사용자가 있을 때(도메인 허용 클릭) 진행. **경남 착수 시 합천은 이미 조사
     완료**(eminwon.hc.go.kr — 지정공고 2018·2021·2024 3년 간격 3회차,
     lastBid 2024-05-13+term 3 후보, §⑲·scratchpad gangwon_findings.md).
     강원 최종: 날짜/약정 확보 9곳(춘천·속초·홍천·평창·정선·철원·인제·고성 +
     양양 contractEnd), 진행 중 3곳(춘천 08-06·삼척 08-10·양양 08-04 — **2026
     하반기 강원 교체기**), 접속 불가 3곳(태백·횡성 타임아웃, 화천 보안 DNS
     NXDOMAIN).
  8. **첨부 hwp 승격 후보**(§⑰ 남은 일 16) — 울산 남구·인천 서해·제물포 ·
     속초(2026-06-25 지정공고 hwpx) · 삼척(2026-08-10 신청안내서 hwpx) ·
     **고성(2026-06-23 지정공고 hwpx) · 양양(평가 배점표 별표1·2 hwpx — RFP
     배점표 관심사와 직결)** 추가. 부산 서구 선례(hwp PrvText 추출, 다운로드는
     사용자 승인)로 승격 가능. 삼척은 WAF가 POST 차단 — 상세·다운로드 모두 GET로.
