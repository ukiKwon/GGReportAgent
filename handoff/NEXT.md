...
### 14. 전국 지자체 금고 입찰 일정 대시보드 — 광역·서울·경기 완료, 부산부터 이어감 (신규, 2026-08-10)

- **출처**: `2026-08-10_summary.md` `## Session 00:41`(경기 웹 검색까지) ·
  `## Session 09:43`(경기 eminwon 전수 조회). 사용자 지시: 서울→경기→부산 순.
- **스펙(전체 이력·근거)**: `docs/superpowers/specs/2026-08-09-nationwide-treasury-schedule-design.md`
  §①~⑫ — 이 항목을 재개하기 전에 §⑨(eminwon 조사법)·§⑩(서울 결과)·§⑪(경기 결과)·
  §⑫(경기 eminwon 결과 + 상세 페이지 호출법)를 먼저 읽을 것. 아래는 재개에 필요한
  핵심만 요약한다.
- **완료 (main, 전부 push됨)**: 광역 17개 시·도(`4235be7`) + 서울 25개 자치구
  (`dc1e3b5`·`02bfea7`·`edb4845`·`10893b6`) + 경기 31개 시군/42 폴리곤 레코드
  (`90afeac`) + 경기 26개 시 eminwon 전수 조회(§⑫). 정본은
  `dashboard/data/institutions.js`(+ 파생 CSV
  `dashboard/data/institutions_시도금고.csv`), 회귀 테스트는
  `dashboard/test/institutions_data.test.js`(node 229 passed 기준).
  총 84개 레코드, **날짜 확보 27곳**(광역 9·서울 9·경기 9=수원 4구 포함).
...
- **남은 하위 항목**:
  ...
  4. **부산 16개 구·군 — 착수 전에 `geo/busan.js` 부재부터 해결 필요.**
     데이터를 넣어도 드릴인 화면은 안 생기고 전국 지도 면 색·랭킹에만 반영된다
     (§⑦-6에서 이미 지적된 사실, 이번에 재확인만 했다). 부산 경계 GeoJSON을
     구하거나(공공데이터포털 등) 이 제약을 받아들이고 광역 레코드만 다룰지
     사용자 결정이 먼저 필요.
...
- **다음 세션 착수 순서 추천**: ③이 끝났으므로 사용자 지시 순서상 다음은
  ④(부산)이고, 착수 전에 `geo/busan.js` 부재에 대한 사용자 결정이 먼저 필요하다.
  단, ②(서울 9월 재조회)는 **시점이 고정돼 있어**(9월 중순) 그 시점이 오면
  순서와 무관하게 우선해야 한다. ①(노원·성북)은 크기가 작아 자투리 시간에
  끼워 넣기 좋다.
