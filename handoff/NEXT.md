...
- **무엇인가**: 둘 다 커밋 대상이 아니고 아무것도 막지 않지만, 방치하면 사고가 난다.
  1. **리포 루트의 stale `registry.db`** (7/28 22:20자, 40KB, 기관 25·bid_case 1·
     ...
     것이었다 — 이 리포는 세션끼리 서로의 untracked 파일을 커밋에 쓸어담은 이력이
     실제로 있어서(2026-07-29 `cd06db9`), 40KB 바이너리 DB가 실수로 커밋될 여지가 있었다.
     → **커밋 위험은 2026-08-06에 해소**: `.gitignore`에 루트 한정 `/*.db` 추가
     (파일명 하나가 아니라 같은 종류 전체를 막는다. 하위 디렉터리는 건드리지 않아
     나중에 의도적으로 커밋할 픽스처 DB를 막지 않는다 — 현재 추적 중인 `.db`는
     리포 전체에 하나도 없음을 확인했다).
     → **남은 것은 원래 PC의 실물 파일 삭제뿐**이고, 이건 아무것도 막지 않는다.
     그 PC에서 `ls registry.db`로 확인 후 지우면 이 항목은 완전히 닫힌다.
