...
- **남은 sub-project**:
  ...
  - **5 통합 프런트 → "기관인텔리" 멀티에이전트 협업 시스템으로 재정의**(2026-07-31,
    ...
    - **화면 ① 입찰상황판 확장 (마지막 남은 화면, 다음 1순위)** — **계획 미작성.**
      스펙 §⑩ 재대조에서 ❌/⚠️로 남은 것이 전부 이 하나로 묶인다:
      - **§② 1번** 지도·목록에 bid_case **예상/확정 배지** 표시
      - **§② 5번** **확정 버튼** — `POST /bidcases/{id}/participation-decisions`는 이미
        있고 화면 버튼만 없다
      - **§② 6번** 확정하면 **3·4단계 자동 실행**(지금은 사람이 `POST /run`을 눌러야 함)
      필요한 것: **기관별 bid_case 조회 API가 없다**(현재 `GET /bidcases?team=&assignee=`
      뿐) — 신설 필요. 지도(`dashboard/js/render.js`)를 건드려야 하는 유일한 작업이라
      **"지도 무수정" 제약이 처음으로 풀리는 지점**이다. 사용자 지시 없이 지도를 고치지 말 것.
    - **스펙 §⑩의 나머지 미충족 2건**(둘 다 비차단):
