...
- **남은 하위 항목**:
  ...
  6. **랭킹 TOP5 중복 표시(비차단)** — 경기 ⓐ안(일반구 분할 도시 전부 같은 값)의
     당연한 결과로, 수원처럼 4개 구로 쪼개진 시가 임박 TOP5 티커의 여러 자리를
     차지해 다른 시가 밀려난다. 데이터 문제가 아니라 `render.js`의 랭킹 로직
     문제라 이번 범위 밖으로 남겼다. 사용자가 화면을 보고 거슬리면 그때
     `sortByUrgency`/티커에 시-단위 중복제거를 추가할지 판단.
