...
**보통은 이 파일을 직접 부를 일이 없다** — `py -3 -m server.demo`가 시딩부터 서버
기동까지 한 번에 한다. 데이터만 다시 깔고 싶을 때만 쓴다:

    py -3 -m server.demo_seed                      # 도봉구에 투입(기본 stage 9)
    py -3 -m server.demo_seed --institution nowon --stage 6
    py -3 -m server.demo_seed --clear              # 데모 행·산출물 삭제

기본 대상은 **데모 전용 DB**(`data/demo.db`)다 — 운영 `registry.db`는 건드리지 않는다
(server/demo_paths.py). 통째로 지우려면 `py -3 -m server.demo --reset`.
...
from server.teams import FINAL_APPROVER
from server.demo_paths import DEMO_DB_PATH, DEMO_OUTPUT_ROOT
...
# (`server/upload_check.py` 참고). 예전 v1은 항목마다 같은 값을 복제해 넣었다.
COVERAGE = {
    "version": 2,
    "items": {
...
def seed(db_path: str, output_root: str, institution_id: str, stage: int,
    ...
    try:
        ...
        if row is None:
            raise SystemExit(
                f"기관 '{institution_id}'이 없습니다 — 먼저 `py -3 -m server.seed`를 실행하세요."
            )
        name_ko = row["name_ko"]
    finally:
        conn.close()
...
def main() -> None:
    ...
    from server.demo import _force_utf8_stdout   # cp949 콘솔에서 죽지 않게(demo.py와 동일)
    _force_utf8_stdout()
