"""데모 환경 한 방에 띄우기 — 시딩 + 데모 투입 + 서버 기동.
    ...
    py -3 -m server.demo                  # http://localhost:8000/ 로 바로 접속
...
(server/demo_paths.py), 그 경로는 `create_app`에 직접 넘긴다 — 환경변수를 거치지
...
둘 중 하나로 해결하세요.
  ...
  2) 패키지가 있는 파이썬으로 실행: py -3.14 -m server.demo
...
def reset() -> list[str]:
    ...
    from server.demo_paths import DEMO_ARTIFACTS
...
def build_demo_index() -> str | None:
    ...
    """
    from server.demo_paths import DEMO_INDEX_DB_PATH, SOURCE_INDEX_DB_PATH
...
def build_app(institution: str = "dobong", stage: int = 9):
    ...
    app = create_app(
        ...
        static_dir=str(REPO_ROOT / "frontend"),
...
def main() -> None:
    ...
    print(
        ...
        f"  지우기  : py -3 -m server.demo --reset --no-serve"
    )
    if args.no_serve:
        return
