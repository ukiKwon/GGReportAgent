...
from server.orchestrator_service import OrchestratorService
from server.routers.approvals import router as approvals_router
from server.routers.bidcases import router as bidcases_router
from server.routers.chat import router as chat_router
from server.routers.inbox import router as inbox_router
from server.routers.accounts import router as accounts_router
from server.routers.consistency import router as consistency_router
from server.routers.documents import router as documents_router
from server.routers.institutions import router as institutions_router
from server.routers.llm_status import router as llm_status_router
from server.routers.menus import router as menus_router
...
def create_app(
    ...
    if static_dir:
        if os.path.isdir(static_dir):
            ...
            # repo 루트 밖에서 기동하면 frontend/가 안 보인다 — API까지 죽이지는 않는다.
            print(f"[warn] 정적 디렉터리를 찾지 못해 마운트를 건너뜁니다: {static_dir}")
    return app


app = create_app(os.environ.get("REGISTRY_DB_PATH", "data/registry.db"), static_dir=os.environ.get("STATIC_DIR", "frontend"))
