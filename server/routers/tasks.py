...
from server.agent_adapter import failure_notice, stream_chat_reply
...
from server.task_repository import (
    add_message,
    approve_task,
...
from server.upload_check import check_upload, write_coverage_map
...
def get_handoff(task_id: str, request: Request) -> dict:
    ...
    **다만 에이전트 전용 단계(RFI분석·취합·검증)는 뺀다** — 그쪽도 tasks 행을 갖지만
    ...
    teams = [{
        ...
        # 팀명→쪽지 수신자 변환은 서버가 한다(server/teams.py) — 화면이 '영업'+'팀'
...
# ── 작업물 파일 (계획 H Task 4) ────────────────────────────────────────
# 경로 위생·확장자·용량은 전부 server/task_files.py가 본다. 여기서는 권한과
# HTTP 상태 코드만 다룬다.
...
def post_task_upload(
    ...
    try:
        ...
        reset_last_model()          # 이 요청에서 LLM이 돌았는지 새로 센다
        ...
        add_message(conn, task_id, "agent", summary, author="검증 agent",
                    model=(last_used_model() or current_model()) if result["llm_used"] else None)
...
def post_task_message(
    ...
    def event_stream():
        # 추적기는 스레드 로컬이라 **스트림이 실제로 도는 이 스레드**에서 초기화한다
        # (요청 스레드에서 부르면 다른 스레드일 때 값이 안 잡힌다).
        reset_last_model()
        reply_parts = []
        completed = False
        failure = None
            ...
            if full_reply:
                ...
                try:
                    ...
                    add_message(write_conn, task_id, "agent", full_reply,
                                model=(last_used_model() or current_model()) if reply_parts else None)
                    if completed:
