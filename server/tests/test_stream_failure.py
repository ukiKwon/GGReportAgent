...
from server.agent_adapter import failure_notice
...
def test_대화_실패시_빈_응답_대신_사유가_스트림된다(client):
    with mock.patch(
        "server.routers.chat.stream_consult_reply",
...
def test_부분_응답이_있으면_사유와_함께_남긴다(client):
    ...
    with mock.patch("server.routers.chat.stream_consult_reply", side_effect=half_then_fail):
...
def test_팀_대화도_실패_사유를_보여준다(client, tmp_path):
    ...
    with mock.patch(
        "server.routers.tasks.stream_chat_reply",
        side_effect=lambda *a, **k: (_ for _ in ()).throw(FakeNotFound()),
    ):
        r = client.post(
