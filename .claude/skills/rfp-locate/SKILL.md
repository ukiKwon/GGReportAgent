...
4. **Structure the scoring table** — from the extracted (or vision-read) text, produce a JSON object matching `references/scoring_schema.json`'s shape: `institution`, `rfp_title`, `total_score`, and a `criteria` list of `{category, item, score, description}`. The raw extracted text does not preserve table column boundaries (verified on the sample RFP: the scoring table's 항목/세부항목/배점/비고 columns collapse into one text run) — this structuring step requires reading the text and reconstructing the table, not a mechanical parse.
   - **Optional `level` field**: if the table is hierarchical (계 → 대분류 → 세부항목), each criteria row may carry `level`: `1` (계/total row), `2` (대분류), `3` (세부항목). Omit it entirely for flat tables — old files without `level` remain valid. Only rows that actually exist in the table get a level; never duplicate a score across levels that the table doesn't show. This matches the automated path (`agent/nodes/rfp_extract.py`) — the two paths must keep the same shape (spec: `docs/superpowers/specs/2026-08-10-scoring-schema-hierarchy-design.md`).
5. **Save outputs**:
