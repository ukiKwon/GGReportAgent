...
| **4 · 결재 흐름 + 오케스트레이터** | 🔶 **① 공고생성·참여결정 완료**(골든 `10`~`14`, §7-5). ② 작업 결재(`15`~`23`) · ③ finalize+조회(`24`~`30`) 남음. `ORCH_RUN`/`ORCH_STEP`은 그 뒤 |
| 5 · 산출물 | ⬜ 미착수 |
...
### 7-5. 결재 흐름 쓰기 — ① 공고생성·참여결정 (단계 4)

골든 `10`~`14`를 **순서대로** 재생한다(`GoldenWriteScenarioTest`). 조회 골든과 다른 점 셋:
