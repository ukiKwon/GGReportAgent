...
@patch("agent.nodes.rfp_extract.structured_llm")
def test_writes_both_artifacts_and_fills_state(mock_structured, tmp_path):
    mock_structured.return_value = _mock_llm([CRITERION])
...
@patch("agent.nodes.rfp_extract.structured_llm")
def test_saved_json_matches_scoring_schema_shape(mock_structured, tmp_path):
...
@patch("agent.nodes.rfp_extract.structured_llm")
def test_empty_scoring_table_is_a_valid_result(mock_structured, tmp_path):
    """배점표 없는 공고문도 있다 — 빈 목록은 실패가 아니다(SKILL.md와 같은 규칙)."""
    mock_structured.return_value = _mock_llm([], total_score=0)
...
def test_abnormal_pdf_stops_and_writes_nothing(mock_extract, mock_structured, tmp_path):
