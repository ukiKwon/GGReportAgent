...
여기서 backend를 import하는 것은 **계약 검증 목적**이라 허용된다.
collector 런타임 코드의 backend/agent import 금지는 test_boundary.py가 강제한다.
"""
...
def test_backend_upsert_accepts_the_rows(csv_path, tmp_path):
    ...
    from backend.repository import list_institutions, upsert_institution
...
def test_dashboard_parser_reads_all_twelve_columns(csv_path, tmp_path):
    ...
    logic_js = Path(__file__).resolve().parents[2] / "dashboard" / "js" / "logic.js"
    ...
    mapo = records["마포구청"]
    assert mapo["subRegion"] == "11140"      # backend가 안 읽는 열
