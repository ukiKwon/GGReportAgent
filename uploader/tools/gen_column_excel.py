...
INFOTYPE = {
    ...
    "사용자번호7": ("CHARACTER", 7, None),
    "년4":        ("CHARACTER", 4, None),
    "년월일8":    ("CHARACTER", 8, None),
    "일시14":     ("CHARACTER", 14, None),
    "내용600":    ("VARCHAR", 600, None),
}
...
ROWS = [
    ("TSKGIAF01", "업로드파일일련번호", "업로드파일일련번호", 1, "일련번호16", "NOT NULL",
     ...
     "기본키. 애플리케이션이 시퀀스 SQ_KGI_AGENTFILETABLE 에서 채번해 직접 입력한다(IDENTITY 미사용)"),
    ("TSKGIAF01", "기관명", "기관명", 0, "명40", "NULLABLE",
     "파일을 제출한 기관의 이름",
     "TSKGIAF02.기관명 과 문자열로 대조해 분류한다. 외래키는 걸지 않으며, "
     "TSKGIAF02 에 없는 이름이면 분류되지 않고 분류상태구분이 '01'(미분류)로 남는다"),
    ("TSKGIAF01", "기관영업분류구분", "기관영업분류구분코드", 0, "구분코드2", "NULLABLE",
     "기관의 영업 분류 코드. TSKGIAF02.기관영업분류구분 에서 복사되어 들어온다",
    ...
    ("TSKGIAF01", "원본파일명", "원본파일명", 0, "명260", "NOT NULL",
     "사용자가 업로드한 원본 파일명",
     "파일시스템 상한이 255자(NTFS·ext4)인데 인포타입에 255 가 없어 260 으로 잡았다"),
    ("TSKGIAF01", "업로드일시", "업로드일시", 0, "일시14", "NOT NULL",
     "파일이 업로드된 시각. **업무 시각**이다",
     "YYYYMMDDHH24MISS 14자. 자릿수가 고정이라 문자열 정렬이 곧 시간 정렬이다"),
    ("TSKGIAF01", "분류일시", "분류일시", 0, "일시14", "NULLABLE",
     "분류가 완료된 시각",
     "YYYYMMDDHH24MISS. 분류 전에는 값이 없어 NOT NULL 을 걸 수 없다"),
    ("TSKGIAF01", "저장경로내용", "저장경로내용", 0, "내용600", "NOT NULL",
     "서버 파일시스템의 저장 경로",
     "분류가 끝나면 값이 바뀐다(미분류 폴더 → classified/{기관영업분류구분}/{문서년}/{기관명}/). "
     "최악 경로가 약 341자다"),
    ("TSKGIAF01", "시스템사용일시", "시스템사용일시", 0, "일시14", "NULLABLE",
     "이 행을 마지막으로 건드린 시각. 업로드일시(업무 시각)와 다른 **감사 시각**이다",
     "INSERT 는 업로드일시와 같은 값, 분류 UPDATE 는 분류일시와 같은 값, "
     "삭제·반려는 그 시점의 DB 시각을 넣는다"),
    ("TSKGIAF01", "시스템사용자번호", "시스템사용자번호", 0, "사용자번호7", "NULLABLE",
     "이 데이터를 넣게 만든 주체. 사람이 아니라 프로그램이다",
     "화면번호에서 앞 K 를 뗀 7자(KGI11100 → GI11100). 재분류 배치는 'BATCH01'. "
     "컨트롤러 클래스명에서 자동으로 도출한다"),

    ("TSKGIAF02", "기관일련번호", "기관일련번호", 1, "일련번호16", "NOT NULL",
     "기관 1건을 식별하는 일련번호",
     "기본키. 애플리케이션이 시퀀스 SQ_KGI_AGENTGIGANTABLE 에서 채번해 직접 입력한다"),
    ("TSKGIAF02", "기관명", "기관명", 0, "명40", "NOT NULL",
     "기관의 이름",
     "UNIQUE 제약. TSKGIAF01.기관명 과 대조하는 실질 키라 중복이 있으면 분류가 어긋난다"),
    ("TSKGIAF02", "기관영업분류구분", "기관영업분류구분코드", 0, "구분코드2", "NOT NULL",
     "기관의 영업 분류 코드. 이 값이 파일의 분류 결과가 된다",
     "01 지방자치단체 / 02 공공기관 / 03 대학교 / 04 병원 / 05 법원"),
    ("TSKGIAF02", "협약기관상태구분", "협약기관상태구분코드", 0, "구분코드2", "NULLABLE",
     "해당 기관과 협약이 체결되었는지 여부",
     "01 미협약 / 02 협약. 애플리케이션이 읽지도 쓰지도 않는다 — 미리 담아 둘 자리다"),
    ("TSKGIAF02", "입찰공고상태구분", "입찰공고상태구분코드", 0, "구분코드2", "NULLABLE",
     "해당 기관의 입찰공고 진행 상태",
     "01 공고전 / 02 공고중 / 03 공고완료. 애플리케이션 미사용(위와 같음)"),
    ("TSKGIAF02", "입찰공고시작년월일", "입찰공고시작년월일", 0, "년월일8", "NULLABLE",
     "입찰공고 시작일", "YYYYMMDD 8자리. 애플리케이션 미사용(위와 같음)"),
    ("TSKGIAF02", "입찰공고마감년월일", "입찰공고마감년월일", 0, "년월일8", "NULLABLE",
     "입찰공고 마감일", "YYYYMMDD 8자리. 애플리케이션 미사용(위와 같음)"),
    ("TSKGIAF02", "시스템사용일시", "시스템사용일시", 0, "일시14", "NOT NULL",
     "기관 정보를 마지막으로 건드린 시각",
     "YYYYMMDDHH24MISS 14자. **옛 `수정일시` 가 이 이름으로 바뀐 것**이라 "
     "TSKGIAF01 쪽과 달리 NOT NULL 이다"),
    ("TSKGIAF02", "시스템사용자번호", "시스템사용자번호", 0, "사용자번호7", "NULLABLE",
     "이 데이터를 넣게 만든 주체",
     "화면번호에서 앞 K 를 뗀 7자(KGI13100 → GI13100). 대량 반입은 GI13400·GI13410"),
]
