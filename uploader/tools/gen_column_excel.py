...
ROWS = [
    ...
    ("TSKGIAF01", "기관영업분류코드", "기관영업분류코드", 0, "코드2", "NULLABLE",
     "기관의 유형 코드. TSKGIAF02.기관영업분류코드 에서 복사되어 들어온다",
    ...
    ("TSKGIAF01", "저장경로", "저장경로내용", 0, "내용600", "NOT NULL",
     "서버 파일시스템의 저장 경로",
     "분류가 끝나면 값이 바뀐다(미분류 폴더 → classified/{기관영업분류코드}/{문서년}/{기관명}/). "
     "기관명이 40자로 줄어 최악 경로가 약 341자다. 인포타입에 400 이 없어 600 으로 잡았다"),
    ...
    ("TSKGIAF02", "기관영업분류코드", "기관영업분류코드", 0, "코드2", "NOT NULL",
    ...
    ("TSKGIAF02", "협약기관상태구분", "협약기관상태구분코드", 0, "구분코드2", "NULLABLE",
     ...
     "01 미협약 / 02 협약. ⚠️ 애플리케이션이 읽지도 쓰지도 않는다 — 기관 정보를 미리 담아 둘 자리다"),
    ("TSKGIAF02", "입찰공고상태", "입찰공고상태구분코드", 0, "구분코드2", "NULLABLE",
     "해당 기관의 입찰공고 진행 상태",
     "01 공고전 / 02 공고중 / 03 공고완료. ⚠️ 애플리케이션 미사용(위와 같음)"),
    ("TSKGIAF02", "입찰공고시작", "입찰공고시작년월일", 0, "년월일8", "NULLABLE",
     "입찰공고 시작일",
     "YYYYMMDD 8자리. ⚠️ 애플리케이션 미사용(위와 같음)"),
    ("TSKGIAF02", "입찰공고마감", "입찰공고마감년월일", 0, "년월일8", "NULLABLE",
     "입찰공고 마감일",
     "YYYYMMDD 8자리. ⚠️ 애플리케이션 미사용(위와 같음)"),
    ("TSKGIAF02", "수정일시", "수정일시", 0, "일시14", "NOT NULL",
     "기관 정보가 마지막으로 수정된 시각",
     "YYYYMMDDHH24MISS 14자. 인포타입 길이가 형식과 정확히 일치한다"),
]
