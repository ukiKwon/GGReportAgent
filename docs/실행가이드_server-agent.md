...
- **작성일**: 2026-07-27
- **대상**: `server/`(레지스트리 API, sub-project 0)와 `agent/`(RFP 자동화 파이프라인, 부분 구현)를
  로컬에서 기동/시험해보려는 사람.
- **범위 밖**: `corpus/reports/`(정적 오프라인 HTML)는 서버 기동이 필요 없음 — 브라우저에서 파일을 직접 열면 됨.

---

## 1. 레지스트리 API (`server/`) — 바로 기동 가능
...
- 테스트만 먼저 확인하고 싶다면: `py -3 -m pytest server/tests -v`
...
py -3.14 -m uvicorn server.main:app --port 8000
...
`agent/orchestrator/graph.py`의 langgraph 그래프를 `server/`에서 백그라운드로
...
py -3 -m uvicorn server.main:app --port 8000
...
연기됐다**(사용자 결정 2026-07-31, `frontend/index.html`에 비활성 버튼만
...
- `server/`: 위 curl 두 개가 200과 JSON을 반환하면 정상.
- `agent/`: `pytest agent/tests -v` 통과 여부로 개별 노드 건전성만 확인 가능;
  end-to-end 실행은 위 제약 때문에 완전한 검증이 아님.
- 오케스트레이터: `pytest server/tests/test_api_workflow.py -v` — subagent만
  목(mock)하고 그래프·게이트·체크포인터는 실물로 돌려 승인 3회→stage 9,
  실패 시 running=false 유지를 검증한다. 로컬 Ollama 실측은 위 §6 절차로
  별도 수행(모델 배선 확인용, CI 대상 아님).
- §7(대화·업로드·완료): `pytest server/tests/test_api_chat.py server/tests/test_api_upload.py server/tests/test_api_complete.py -v`.
...
## 8. 대시보드 (`frontend/`) — 서버 모드 (정적 자산 마운트)
...
#   (환경변수 STATIC_DIR로 경로 교체 가능, 기본값: frontend/)
...
병합은 **서버가 아니라 클라이언트(`frontend/js/serverdata.js`)에서** 일어난다. 페이지가
항상 로드하는 정적 스크립트 `frontend/data/institutions.js`(`window.institutions`)와
...
CSV 헤더(한글, `server/csv_import.py`의 `HEADER_MAP` 실물): `기관명`, `기관구분`,
...
| `STATIC_DIR` | `frontend/` | 정적 자산(index.html, js/, 등) 마운트 경로 |
...
```bash
py -3 -m pytest server/tests/test_static_e2e.py -v
```

테스트 항목:
- 실제 frontend/index.html 로드 (GET / → \<title> 포함 확인)
...
`file://`로 `frontend/index.html`을 직접 열면 오케스트레이터 API가 없으므로 탭 버튼 자체가
보이지 않는다. §8대로 서버를 띄우고 `http://localhost:8000/` 로 접속해야 나타난다.

```bash
py -3 -m uvicorn server.main:app --port 8000   # 저장소 루트에서
...
```bash
py -3 -m server.demo          # 시딩 + 데모 투입 + 서버 기동까지 한 번에
# → http://localhost:8000/  에서 [워크플로] 탭
```

```bash
py -3 -m server.demo --port 8123
py -3 -m server.demo --institution nowon --stage 6
py -3 -m server.demo --reset               # 데모 자료를 통째로 지우고 새로 만든다
py -3 -m server.demo --reset --no-serve    # 지우기만
```

**데모는 개발/운영 자료와 파일이 갈려 있다**(`server/demo_paths.py`). 데모가 운영
...
| 실행 | `py -3 -m uvicorn server.main:app` | `py -3 -m server.demo` |
...
- **파이썬이 여러 개 깔린 PC**에서는 `py -3`가 패키지 없는 버전을 가리킬 수 있다.
  그럴 때 `server.demo`는 traceback 대신 무엇을 하면 되는지(설치 명령 / 다른 런처로
...
**계정 전환(데모 전용)** — `server.demo`로 띄우면 옆에 `계정 전환…` 드롭다운이 하나 더
...
```bash
py -3 -m server.demo          # 시딩 + 데모 투입 + 서버 기동
```
...
> 지도 코드(`frontend/js/render.js`)는 **한 줄도 고치지 않았다.** 임박도 색이 원래
...
`py -3.14 -m server.demo`는 기동할 때 운영 인덱스를 **`data/demo_corpus_index.db`로
...
- **오른쪽 위: 이관 패키지** — 그 공고의 팀별 카드. 여기가 **디자이너가 받는 것**이다.
  ...
  - 카드의 **[문의]** 를 누르면 쪽지함 발송 폼이 **수신자가 채워진 채** 열린다.
    ...
    `server/teams.py` 한 곳에만 있다). 보낸 쪽지에는 기관·작업 링크가 함께 실린다.
...
- **[제출]** 은 두 조건이 다 맞아야 눌린다. 못 누를 때는 **버튼 옆에 이유가 뜬다.**
  ...
  - 💡 **데모에서 제출까지 눌러 보려면**: 기본 시드는 영업이 결재 대기·예산이 작성
    ...
    `py -3.14 -m server.demo_seed --teams-done` 을 돌리고 새로고침해도 된다
...
- 저장·전송은 여전히 **합쳐진 문자열 하나**다(`영업팀`·`영업팀장`·`영업부장`·
  ...
  키가 전부 갈라진다. 규칙은 `server/teams.py`(`compose`/`split_role`)와
  `frontend/js/roles.js` 두 곳에 같은 답으로 있다.
...
- `영업팀`·`영업팀장`·`영업부장`은 **같은 `tasks.team`(`영업`)** 을 본다. 접미사를
  떼는 규칙은 `server/teams.py`의 `team_of` 한 곳에만 있다.
...
- 저장된 값이 없으면 **코드의 기본값**(`server/menus.py`)이 쓰인다. 행이 없다는 것은
