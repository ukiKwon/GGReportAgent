...
- **[제출]** 은 두 조건이 다 맞아야 눌린다. 못 누를 때는 **버튼 옆에 이유가 뜬다.**
  ...
  누르면 상태가 `제출됨`이 되고 **본부장에게 결재요청**이, **각 팀에는 전달 쪽지**가 간다.
...
```
팀원 ──제출──▶ 그 팀의 팀장 ──승인──▶ (3팀 전부 승인) ──▶ 디자이너 ──제출──▶ 본부장
   (작업함)         (결재함)                              (작업함)      (결재함)
```

- **팀 작업물은 그 팀 팀장이 결재한다.** 남의 팀을 대신 보지 않는다 — 겹치면 누가
  봤는지 알 수 없어진다.
- **디자이너 작업물과 8단계 최종결재는 본부장이 한다.** 본부장은 팀 작업을 대신
  결재하지 않는다.
- **반려하면 담당자에게 쪽지가 간다.** 예전에는 상태만 `작업 중`으로 되돌리고
  아무도 몰랐다. 사유 없이 반려하면 화면이 한 번 되묻는다.
- ⚠️ `인사권자`는 **`본부장`으로 개명**됐다. 개명 전에 쌓인 알림은 옛 이름으로 남아
  있는데, 조회할 때 같은 것으로 본다(`teams.recipient_aliases`).

### 16-2. 소속(역할)과 기본 화면

소속은 상단바 프로필에서 고른다. **소속이 곧 역할**이다(별도 직책 필드가 없다).

| 소속 | 기본으로 보이는 탭 |
|---|---|
| `영업팀`·`전산팀`·`예산팀` | 지도 · 지역별 · 워크플로 · 대화 · 지식 · **작업함** |
| `영업팀장`·`전산팀장`·`예산팀장` | 위 + **결재함** |
| `디자이너` | 지도 · 지역별 · 대화 · 지식 · **작업함** |
| `본부장` | 지도 · 지역별 · **결재함** (워크플로 없음 — 결재에 필요한 맥락은 카드 안에 있다) |
| `전산팀` | 팀원 기본 + **권한관리** (시스템 운영자를 겸한다) |

- **작업함은 디자이너 전용이 아니다.** 팀원이 열면 자기 팀 작업이 뜨고, 이관 패키지에
  **다른 팀 산출물과 디자이너 작업물**이 함께 보인다(자기 팀만 빠진다).
- `영업팀`과 `영업팀장`은 **같은 `tasks.team`(`영업`)** 을 본다. 접미사를 떼는 규칙은
  `backend/teams.py`의 `team_of` 한 곳에만 있다.
...
4. `본부장`으로 전환 → 디자이너 제출물과 최종결재 게이트가 결재함에 뜬다.
   **워크플로 탭이 없다.**
5. `전산팀`으로 전환 → 권한관리에서 `본부장 × 워크플로`를 켜고 저장 → 본부장으로
   전환하면 워크플로 탭이 생긴다.
