...
1. **대화 탭 배지** (`#chat-model-badge`, §10) — 기관을 고르면 입력창 위에
   `🧠 <model>` 배지가 붙는다. 데이터원은 `GET /llm/status`이고 문구는
   ...
   조회 실패 시(엔드포인트가 죽어 있는 등) 배지는 조용히 사라진다 — 부가 정보라
   대화 자체를 막지는 않는다.
2. **지식 탭 검색 모드·임베딩 모델** (§13-4) — 검색 결과 머리글에 두 표시가 나란히
   ...
   정확하고 모델명까지 준다.
3. **워크플로 로그의 🧠 표시** (§9, `GET /tasks/{id}`) — 참여자 카드를 눌러 여는
   팀별 작업 로그에서, LLM을 호출해 남긴 기록에는 시각 뒤에 `· 🧠 <model>`이
   붙는다(`messages.model` 컬럼 → `workflow.logRows` → 렌더). 사람 발화나 게이트
   통과 알림처럼 LLM을 안 쓴 기록에는 **아무것도 안 붙는다** — 값이 없을 때만
   생략되므로 비LLM 기록이 지저분해지지 않는다.
   - **스테퍼 칸을 눌러 보는 단계 전체 로그**(`GET /institutions/{id}/timeline`)는
     아직 이 표시가 없다 — `timeline` 응답에 `model` 필드가 없기 때문이다. 모델
     확인이 필요하면 참여자 카드를 눌러 팀별 로그(`GET /tasks/{id}`)로 들어갈 것.
