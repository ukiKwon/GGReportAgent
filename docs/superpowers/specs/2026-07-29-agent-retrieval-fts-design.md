...
- trigram 토크나이저 특성: 질의가 **3자 미만이면 매치 불가** → 빈 목록 반환(예외
  아님). 자연어 질의는 **토큰(3자 이상) + 인접 토큰쌍을 각각 구문 리터럴(`"…"`)로
  감싸 OR로 연결**한다 — 질의 전체를 한 구문으로 감싸면 채팅 문장이 코퍼스에 통째로
  실재해야만 매치되고(구현 중 실측 확인), 한국어 2자 단어(청년·창업 등)는 인접쌍
  구문("청년 창업")으로만 살릴 수 있다. 문법 문자(따옴표 등)는 리터럴 내부로 격리된다.
