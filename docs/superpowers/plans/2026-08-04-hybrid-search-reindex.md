...
## 실행 결과 (2026-08-04) — 계획과 달라진 점

전 태스크 완료. 테스트 **408 → 478 passed**, dashboard **100 → 107**.
브랜치 `hybrid-search-f`.

**계획을 벗어난 판단 5건 (전부 의도적, 이유 포함)**

1. **`build_index(embed=)`의 기본값을 켜짐 → 꺼짐으로 뒤집었다.** 계획서는 켜짐이었는데,
   개발 PC에 Ollama가 **실제로 떠 있어서** 인덱스를 만드는 기존 테스트들이 전부
   실호출로 수 초씩 잡아먹고 환경에 따라 결과가 달라진다. 켜는 것은 CLI와 재색인
   서비스의 책임으로 옮겼다(`reindex`는 반대로 기본 켜짐 — 다른 테스트가 암묵적으로
   부르지 않는 명시적 유지보수 동작이라 그렇다).
2. **`.pptx` 파서를 새로 넣었다(계획에 없음).** 파서가 `.txt`뿐이라 아카이브에서
   색인되는 파일이 `rfp_text.txt` **하나뿐**이었다. 정작 진짜 산출물인 제안서 pptx가
   빠지면 §② 17이 이름뿐인 약속이 된다. python-pptx는 이미 의존성에 있었다.
3. **전체 재빌드가 아카이브를 날리는 구멍을 추가로 막았다(계획에 없음).**
   `build_index`는 인덱스를 통째로 새로 만드는데 스캔 루트가 `corpus/` 하나라,
   `build`를 한 번 돌리면 완료 산출물이 검색에서 조용히 사라졌다 — §② 17이 재빌드
   때마다 깨지는 셈이었다. CLI `build`가 아카이브를 이어서 색인하게 했다.
4. **`load_institution_names(registry_db)`를 `agent/` 쪽에 뒀다.** 아카이브 폴더명이
   한글 `name_ko`라 슬러그로 되짚어야 하는데, `agent/`는 `backend/`를 import하지 않는다.
   코드가 아니라 **파일**을, 그것도 `institution_id`·`name_ko` 두 컬럼만 읽는다 —
   이 둘은 `collector/SCHEMA.md`가 망 경계 계약으로 이미 고정한 이름이라 임의로 바뀌지
   않는다. 파일이 없거나 DB가 아니어도 빈 매핑으로 넘어간다.
5. **임베딩이 중간에 끊겨도 만들어둔 벡터를 버리지 않는다.** 계획서는 실패 시
   `vectors`를 비우는 쪽이었는데, 57분짜리가 41분째에 끊겼다고 처음부터 다시 시킬
   이유가 없다. `_embed_chunks`가 벡터 없는 청크만 LEFT JOIN으로 골라 이어 채운다.

**계획대로 간 것 중 확인해둘 것**

- `dashboard/js/render.js` **무수정** — 계획 A1 이후 한 번도 깨지지 않은 제약이
  이번에도 유지됐다(`git diff main...HEAD -- dashboard/js/render.js` 빈 출력).
- `search()` 시그니처 유지 — `backend/routers/search.py`·`backend/agent_adapter.py`
  한 줄도 안 고쳤다. `RetrievedChunk`에 붙인 필드는 기본값이 있어 `asdict`로 API
  응답에 자동 반영된다.
- 원자 교체 유지, 임베딩 부재 시 FTS 단독 자동 폴백, 테스트는 Ollama 없이 통과.

**미결로 남긴 것**

- 전체 인덱스 실빌드는 이 세션에서 백그라운드로 돌렸다(약 57분). 완료 후 브라우저
  확인(Verification 1~6)이 남는다.
- 벡터 캐싱 없음 — 질의마다 `vectors` 전체를 읽어 행렬을 만든다(2,763×1024 = 11MB).
  질의 임베딩 1.2초에 비하면 무시할 수준이라 일부러 안 넣었다. 수만 건이 되면
  `sqlite-vec` 도입과 함께 재검토.
