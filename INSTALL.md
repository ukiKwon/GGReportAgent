...
> **대안: `LLM_MODEL=auto`.** 위 서비스 파일은 **명시 모델값이 기본**이다 — 폐쇄망
> 운영에서는 "지금 이 서버가 정확히 어느 모델을 쓰는지"를 코드/설정만 보고 알 수
> 있어야 하므로, 여러 인스턴스에 같은 값을 박아 넣는 게 원칙이다. 다만 인스턴스마다
> 사양이 달라 매번 손으로 골라야 하는 상황이라면 `Environment=LLM_MODEL=auto`로
> 바꿔도 된다. `auto`가 하는 일(`agent/model_select.py`):
> - 이 서버의 **RAM·vCPU**(`detect_resources()`)와 **Ollama에 실제로 설치된 모델
>   목록**(`GET /api/tags`)의 교집합에서 가장 큰 것을 고른다.
> - 후보는 **3종 고정**: `llama3.1:8b`(RAM 7GB+·4 vCPU+) → `llama3.2:3b`
>   (3.5GB+·2 vCPU+) → `llama3.2:1b`(1.8GB+·1 vCPU+). 이 셋 밖의 모델은 자동선택
>   대상이 아니다.
> - 판정은 프로세스당 1회만 하고 캐시한다(추론마다 Ollama를 다시 찌르지 않는다).
> - **못 고르면**(하드웨어가 셋 다 못 감당하거나 Ollama에 아무것도 안 받아져 있으면)
>   기본값(`gpt-oss-120b`)으로 **폴백**하고 stderr에 경고를 남긴다:
>   `[llm] LLM_MODEL=auto인데 쓸 모델을 못 찾았습니다 — …`. 이 경고가 보이면
>   `ollama pull llama3.2:3b` 등으로 후보 하나를 받아야 실제로 auto가 동작한다.
> - 화면에서 지금 무슨 모델이 골라졌는지 확인하는 법은
>   `docs/실행가이드_backend-agent.md` §14 참고(대화 탭 배지에 `자동 선택 · RAM …`
>   문구가 붙는다).
